...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    if (bbMatches.size() > 0) {
        meanDistance = meanDistance / bbMatches.size();
    } 
    else {
        return;
    }
...
{
    cv::KeyPoint prevPoints;
    cv::KeyPoint currPoints;
    int prevBBId;
    int currBBId;
    int currBBSize;
    int prevBBSize;
    int bestBB;
    std::map<int,int> boxMap;
    std::vector<int> currBoxCount;

    // find all the bounding boxes for current and previous frame
    for(auto match:matches){
        currBBId = -1;
        prevBBId = -1;

        prevPoints = prevFrame.keypoints[match.queryIdx];
        currPoints = currFrame.keypoints[match.trainIdx];
        ...
        boxMap.insert({currBBId, prevBBId});
    }

    for (int i = 0; i < currBBSize; i++){
        auto bbPair = boxMap.equal_range(i);
        std::vector<int> currBoxCount (prevBBSize, 0);

        for(auto ptr = bbPair.first; ptr != bbPair.second; ptr++){
            if((*ptr).second != -1)
                currBoxCount[(*ptr).second] += 1;
        }

        bestBB = std::distance(currBoxCount.begin(), std::max_element(currBoxCount.begin(), currBoxCount.end()));

        bbBestMatches.insert({bestBB, i});
        cout << "Current BBID: " << i << "matches previous BBID: "<< bestBB;
    }
